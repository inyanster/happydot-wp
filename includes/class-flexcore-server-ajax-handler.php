...
class FlexCore_Server_Ajax_Handler
{
    ...
    private function register_ajax_endpoints()
    {
        ...
        add_action('wp_ajax_nopriv_flexcore_myinfo_unbind', array($this, 'handle_myinfo_unbind'));
        add_action('wp_ajax_flexcore_myinfo_complete_profile', array($this, 'handle_myinfo_complete_profile'));
        add_action('wp_ajax_nopriv_flexcore_myinfo_complete_profile', array($this, 'handle_myinfo_complete_profile'));
    ...
    //     ));
    //     check_ajax_referer('flexcore-server-nonce', 'nonce');
...
/**
 * Handle MyInfo profile completion after Singpass callback (existing user)
 */
public function handle_myinfo_complete_profile() {
    check_ajax_referer('flexcore-server-nonce', 'nonce');
    if (!FlexCore_Server_Session::is_authenticated()) {
        wp_send_json_error(['message' => 'You must be logged in.']);
        return;
    }
    $flow_id = isset($_POST['flowId']) ? sanitize_text_field($_POST['flowId']) : '';
    if (empty($flow_id)) {
        wp_send_json_error(['message' => 'Missing flowId.']);
        return;
    }
    $api = new FlexCore_Server_API();
    $result = $api->myinfo_complete_profile($flow_id);
    if (is_wp_error($result)) {
        wp_send_json_error(['message' => $result->get_error_message()]);
    } else {
        wp_send_json_success($result);
