(function ($) {
	...
	const MergedRegistration = {
		...
		handleReferralAndUTM: function () {
			...
			let campaignId = urlParams.get('campaign_id') || 'referralsignup';
			...
			if (utm_string_present && referral_code_absent) {
				...
				if (sourceSelect) {
					sourceSelect.style.require = 'none';
 ...
 let isValidPostalCode = false;
	...
	const form = $("#flexcore-profile-referral-form");
  ...
  return isValidPostalCode;
		...
		validateFields: function () {
			...
			if ($("#referral_code").prop("required") && !$("#referral_code").val().trim()) {
