(function ($) {
  ...
  const MergedRegistration = {
    init: function () {
      ...
      $("#preferred_name").on("input blur", this.validatePreferredName);
      ...
      $("#terms_consent").on("change", this.validateTermsConsent);
...
if (utm_string_present && referral_code_absent) {
  ...
  if (sourceSelect) {
    ...
    validatePreferredName: function () {
      const name = $("#preferred_name").val().trim();
      if (!name) {
        $("#preferred_name").addClass("has-error").removeClass("is-valid");
        $(".preferredName-error").text("Please enter your preferred name").show();
        return false;
      } else if (name.length < 2) {
        $("#preferred_name").addClass("has-error").removeClass("is-valid");
        $(".preferredName-error").text("Preferred name must be at least 2 characters").show();
        return false;
      } else {
        $("#preferred_name").removeClass("has-error");
        $(".preferredName-error").hide();
        return true;
      }
    },
    ...
    validateTermsConsent: function () {
      if (!$("#terms_consent").is(":checked")) {
        $(".terms-error").text("Please agree to the Terms & Conditions").show();
        $("#terms_consent").addClass("has-error");
        return false;
      } else {
        $(".terms-error").hide();
        $("#terms_consent").removeClass("has-error");
        return true;
      }
    },
    ...
    validateFields: function () {
      ...
      if (!this.validatePreferredName()) valid = false;
  ...
  valid = false;
      ...
      if (!this.validateTermsConsent()) valid = false;
      // Date of Birth
      // Mobile
  ...
  function isSequentialRun(digits) {
    ...
    const seq = digits.slice(1);
    if (seq.length < 2) return false;
    const diff = seq.charCodeAt(1) - seq.charCodeAt(0);
  ...
  $(document).ready(function () {
    window.MergedRegistration = MergedRegistration;
